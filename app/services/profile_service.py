...
class ProfileService:
    ...
    def create_new_profile(
        self, user_data: ProfileCreateDTO, user_id: int
    ) -> tuple[UserProfile, AthleteProfile]:
        if self.profile_repo.get_user_profile(user_id):
            raise ProfileAlreadyExistsError
        user_profile = UserProfile(
            id=user_id, name=user_data.name, birth_date=user_data.birth_date, height_cm=user_data.height_cm
        )
        athlete_profile = AthleteProfile(
            id=user_id,
            weight_kg=user_data.weight_kg,
            current_ftp=user_data.current_ftp,
            gear=user_data.gear,
            environment_location=user_data.environment_location,
